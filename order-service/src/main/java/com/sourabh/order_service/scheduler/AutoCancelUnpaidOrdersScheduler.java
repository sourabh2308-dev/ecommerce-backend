...
/**
 * Automatically cancels unpaid orders older than configured threshold.
 * Restores stock from cancelled orders.
 */
@Component
...
public class AutoCancelUnpaidOrdersScheduler {
    ...
    @Value("${scheduler.auto-cancel-unpaid-orders.cron:0 */5 * * * *}")  // Every 5 minutes
    private String cronExpression;

    /**
     * Run at configured interval (default: every 5 minutes).
     * Cancel orders created more than `unpaidOrderThresholdMinutes` (default 30 min) ago with status CREATED.
     */
        ...
        try {
            ...
            for (Order order : expiredOrders) {
                try {
                    ...
                    // TODO: Implement stock restoration from cancelled order items
                    // This should restore inventory for each order item
